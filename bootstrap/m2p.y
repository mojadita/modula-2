...
ConstantDeclaration
		: IDENT '=' ConstExpression {
			$$ = alloc_NONLEAF(CL_ConstantDeclaration, 8, 3, alloc_IDENT($1), alloc_SYMBOL('=', $2), $3);
...
relation
		: '=' {
			$$ = alloc_NONLEAF(CL_relation, 11, 1, alloc_SYMBOL('=', $1));
			RULE(relation, SYMBOL("="));
		}
		| '#' {
			$$ = alloc_NONLEAF(CL_relation, 12, 1, alloc_SYMBOL('#', $1));
			RULE(relation, SYMBOL("#"));
		}
		| NE {
			$$ = alloc_NONLEAF(CL_relation, 13, 1, alloc_SYMBOL(NE, $1));
			RULE(relation, SYMBOL("<>"));
		}
		| '<' {
			$$ = alloc_NONLEAF(CL_relation, 14, 1, alloc_SYMBOL('<', $1));
			RULE(relation, SYMBOL("<"));
		}
		| LE {
			$$ = alloc_NONLEAF(CL_relation, 15, 1, alloc_SYMBOL(LE, $1));
			RULE(relation, SYMBOL("<="));
		}
		| '>' {
			$$ = alloc_NONLEAF(CL_relation, 16, 1, alloc_SYMBOL('>', $1));
			RULE(relation, SYMBOL(">"));
		}
		| GE {
			$$ = alloc_NONLEAF(CL_relation, 17, 1, alloc_SYMBOL(GE, $1));
			RULE(relation, SYMBOL(">="));
		}
		| IN {
			$$ = alloc_NONLEAF(CL_relation, 18, 1, alloc_SYMBOL(IN, $1));
...
SimpleConstExpr
		: ConstTerm_list {
			$$ = alloc_NONLEAF(CL_SimpleConstExpr, 19, 1, $1);
...
add_op_opt
		: '+' {
			$$ = alloc_NONLEAF(CL_add_op_opt, 22, 1, alloc_SYMBOL('+', $1));
			RULE(add_op_opt, SYMBOL("+"));
		}
		| '-' {
			$$ = alloc_NONLEAF(CL_add_op_opt, 23, 1, alloc_SYMBOL('-', $1));
			RULE(add_op_opt, SYMBOL("-"));
		}
		| /* empty */ {
			$$ = alloc_NONLEAF(CL_add_op_opt, 24, 0);
...
AddOperator
		: '+' {
			$$ = alloc_NONLEAF(CL_AddOperator, 25, 1, alloc_SYMBOL('+', $1));
			RULE(AddOperator, SYMBOL("+"));
		}
		| '-' {
			$$ = alloc_NONLEAF(CL_AddOperator, 26, 1, alloc_SYMBOL('-', $1));
			RULE(AddOperator, SYMBOL("-"));
		}
		| OR {
			$$ = alloc_NONLEAF(CL_AddOperator, 27, 1, alloc_SYMBOL(OR, $1));
...
MulOperator
		: '*' {
			$$ = alloc_NONLEAF(CL_MulOperator, 30, 1, alloc_SYMBOL('*', $1));
			RULE(MulOperator, SYMBOL("*"));
		}
		| '/' {
			$$ = alloc_NONLEAF(CL_MulOperator, 31, 1, alloc_SYMBOL('/', $1));
			RULE(MulOperator, SYMBOL("/"));
		}
		| DIV {
			$$ = alloc_NONLEAF(CL_MulOperator, 32, 1, alloc_SYMBOL(DIV, $1));
			RULE(MulOperator, KEYWORD(DIV));
		}
		| MOD {
			$$ = alloc_NONLEAF(CL_MulOperator, 33, 1, alloc_SYMBOL(MOD, $1));
			RULE(MulOperator, KEYWORD(MOD));
		}
		| AND {
			$$ = alloc_NONLEAF(CL_MulOperator, 34, 1, alloc_SYMBOL(AND, $1));
			RULE(MulOperator, KEYWORD(AND));
		}
		| '&' {
			$$ = alloc_NONLEAF(CL_MulOperator, 35, 1, alloc_SYMBOL('&', $1));
			RULE(MulOperator, SYMBOL("&"));
		} ;
ConstFactor
		: qualident {
			$$ = alloc_NONLEAF(CL_ConstFactor, 36, 1, $1);
			RULE(ConstFactor, NONTERM(qualident));
		}
		| INTEGER {
			$$ = alloc_NONLEAF(CL_ConstFactor, 37, 1, alloc_INTEGER($1));
			RULE(ConstFactor, TERMIN("INTEGER(%d)"), $1);
		}
		| REAL {
			$$ = alloc_NONLEAF(CL_ConstFactor, 38, 1, alloc_REAL($1));
			RULE(ConstFactor, TERMIN("REAL(%lg)"), $1);
		}
		| STRING {
			$$ = alloc_NONLEAF(CL_ConstFactor, 39, 1, alloc_STRING($1));
			RULE(ConstFactor, TERMIN("STRING(%s)"), $1);
		}
		| CHARLIT {
			$$ = alloc_NONLEAF(CL_ConstFactor, 40, 1, alloc_CHARLIT($1));
			RULE(ConstFactor, TERMIN("CHARLIT(\\%03d)"), $1);
		}
		| set {
			$$ = alloc_NONLEAF(CL_ConstFactor, 41, 1, $1);
			RULE(ConstFactor, NONTERM(set));
		}
		| '(' ConstExpression ')' {
			$$ = alloc_NONLEAF(CL_ConstFactor, 42, 3, alloc_SYMBOL('(', $1), $2, alloc_SYMBOL(')', $3));
			RULE(ConstFactor, SYMBOL("(") NONTERM(ConstExpression) SYMBOL(")"));
		}
		| NOT ConstFactor {
			$$ = alloc_NONLEAF(CL_ConstFactor, 43, 2, alloc_SYMBOL(NOT, $1), $2);
...
set
		: qualident '{' element_list_opt '}' {
			$$ = alloc_NONLEAF(CL_ConstFactor, 44, 4, $1, alloc_SYMBOL('{', $2), $3, alloc_SYMBOL('}', $4));
			RULE(set, NONTERM(qualident) SYMBOL("{") NONTERM(element_list_opt) SYMBOL("}"));
		}
		| '{' element_list_opt '}' {
			$$ = alloc_NONLEAF(CL_ConstFactor, 45, 3, alloc_SYMBOL('{', $1), $2, alloc_SYMBOL('}', $3));
...
element_list_opt
		: element_list {
			$$ = alloc_NONLEAF(CL_element_list_opt, 46, 1, $1);
			RULE(element_list_opt, NONTERM(element_list));
		}
		| /* empty */ {
			$$ = alloc_NONLEAF(CL_element_list_opt, 47, 0);
...
element_list
		: element_list ',' element {
			$$ = alloc_NONLEAF(CL_element_list, 48, 3, $1, alloc_SYMBOL(',', $2), $3);
			RULE(element_list, NONTERM(element_list) SYMBOL(",") NONTERM(element));
		}
		| element {
			$$ = alloc_NONLEAF(CL_element_list, 49, 1, $1);
...
element
		: ConstExpression RANGE ConstExpression {
			$$ = alloc_NONLEAF(CL_element, 50, 3, $1, alloc_SYMBOL(RANGE, $2), $3);
			RULE(element, NONTERM(ConstExpression) SYMBOL("..") NONTERM(ConstExpression));
		}
		| ConstExpression  {
			$$ = alloc_NONLEAF(CL_element, 51, 1, $1);
...
TypeDeclaration
		: IDENT '=' type {
            $$ = alloc_NONLEAF(CL_TypeDeclaration, 52, 3, alloc_IDENT($1), alloc_SYMBOL('=', $2), $3);
...
type
		: SimpleType {
            $$ = alloc_NONLEAF(CL_type, 53, 1, $1);
			RULE(type, NONTERM(SimpleType));
		}
		| ArrayType {
            $$ = alloc_NONLEAF(CL_type, 54, 1, $1);
			RULE(type, NONTERM(ArrayType));
		}
		| RecordType {
            $$ = alloc_NONLEAF(CL_type, 55, 1, $1);
			RULE(type, NONTERM(RecordType));
		}
		| SetType {
            $$ = alloc_NONLEAF(CL_type, 56, 1, $1);
			RULE(type, NONTERM(SetType));
		}
		| PointerType {
            $$ = alloc_NONLEAF(CL_type, 57, 1, $1);
			RULE(type, NONTERM(PointerType));
		}
		| ProcedureType {
            $$ = alloc_NONLEAF(CL_type, 58, 1, $1);
...
SimpleType
		: qualident {
            $$ = alloc_NONLEAF(CL_SimpleType, 59, 1, $1);
			RULE(SimpleType, NONTERM(qualident));
		}
		| enumeration {
            $$ = alloc_NONLEAF(CL_SimpleType, 60, 1, $1);
			RULE(SimpleType, NONTERM(enumeration));
		}
		| SubrangeType {
            $$ = alloc_NONLEAF(CL_SimpleType, 61, 1, $1);
...
enumeration
		: '(' IdentList ')' {
            $$ = alloc_NONLEAF(CL_enumeration, 62, 3, alloc_SYMBOL('(', $1), $2, alloc_SYMBOL(')', $3));
...
IdentList
		: IdentList ',' IDENT {
            $$ = alloc_NONLEAF(CL_IdentList, 63, 3, $1, alloc_SYMBOL(',', $2), alloc_IDENT($3));
			RULE(IdentList, NONTERM(IdentList) SYMBOL(",") TERMIN("IDENT:%s"), $3);
		}
		| IDENT {
            $$ = alloc_NONLEAF(CL_IdentList, 64, 1, alloc_IDENT($1));
...
SubrangeType
		: '[' ConstExpression RANGE ConstExpression ']' {
            $$ = alloc_NONLEAF(CL_SubrangeType, 65, 5, alloc_SYMBOL('[', $1), $2, alloc_SYMBOL(RANGE, $3), $4, alloc_SYMBOL(']', $5));
...
ArrayType
		: ARRAY SimpleType_list OF type {
            $$ = alloc_NONLEAF(CL_ArrayType, 66, 4, alloc_SYMBOL(ARRAY, $1), $2, alloc_SYMBOL(OF, $3), $4);
...
SimpleType_list
		: SimpleType_list ',' SimpleType {
            $$ = alloc_NONLEAF(CL_SimpleType_list, 67, 3, $1, alloc_SYMBOL(',', $2), $3);
			RULE(SimpleType_list, NONTERM(SimpleType_list) SYMBOL(",") NONTERM(SimpleType));
		}
		| SimpleType {
            $$ = alloc_NONLEAF(CL_SimpleType_list, 68, 1, $1);
...
RecordType
		: RECORD
			FieldListSequence
		  END {
            $$ = alloc_NONLEAF(CL_RecordType, 69, 3, alloc_SYMBOL(RECORD, $1), $2, alloc_SYMBOL(END, $3));
...
FieldListSequence
		: FieldListSequence ';' FieldList {
            $$ = alloc_NONLEAF(CL_FieldListSequence, 70, 3, $1, alloc_SYMBOL(';', $2), $3);
			RULE(FieldListSequence, NONTERM(FieldListSequence) SYMBOL(";") NONTERM(FieldList));
		}
		| FieldList {
            $$ = alloc_NONLEAF(CL_FieldListSequence, 71, 1, $1);
...
FieldList
		: IdentList ':' type {
            $$ = alloc_NONLEAF(CL_FieldList, 72, 3, $1, alloc_SYMBOL(':', $2), $3);
		...
		| CASE case_ident OF
		  ...
		  END {
            $$ = alloc_NONLEAF(CL_FieldList, 73, 6, alloc_SYMBOL(CASE, $1), $2, alloc_SYMBOL(OF, $3), $4, $5, alloc_SYMBOL(END, $6));
			RULE(FieldList, KEYWORD(CASE) NONTERM(case_ident) KEYWORD(OF)
			    NONTERM(variant_list) NONTERM(ELSE_FieldListSequence) KEYWORD(END));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_FieldList, 74, 0);
...
case_ident
		: IDENT ':' qualident  {
            $$ = alloc_NONLEAF(CL_case_ident, 75, 3, alloc_IDENT($1), alloc_SYMBOL(':', $2), $3);
			RULE(case_ident, TERMIN("IDENT:%s") SYMBOL(":") NONTERM(qualident), $1);
		}
		| 			qualident {
            $$ = alloc_NONLEAF(CL_case_ident, 76, 1, $1);
...
variant_list
		: variant_list '|' variant {
            $$ = alloc_NONLEAF(CL_variant_list, 77, 3, $1, alloc_SYMBOL('|', $2), $3);
			RULE(variant_list, NONTERM(variant_list) SYMBOL("|") NONTERM(variant));
		}
		| variant {
            $$ = alloc_NONLEAF(CL_variant_list, 78, 1, $1);
...
ELSE_FieldListSequence
		: ELSE FieldListSequence {
            $$ = alloc_NONLEAF(CL_ELSE_FieldListSequence, 79, 2, alloc_SYMBOL(ELSE, $1), $2);
			RULE(ELSE_FieldListSequence, KEYWORD(ELSE) NONTERM(FieldListSequence));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_ELSE_FieldListSequence, 80, 0);
...
variant
		: CaseLabelList ':' FieldListSequence {
            $$ = alloc_NONLEAF(CL_variant, 81, 3, $1, alloc_SYMBOL(':', $2), $3);
...
CaseLabelList
		: CaseLabelList ',' CaseLabels {
            $$ = alloc_NONLEAF(CL_CaseLabelList, 82, 3, $1, alloc_SYMBOL(',', $2), $3);
			RULE(CaseLabelList, NONTERM(CaseLabelList) SYMBOL(",") NONTERM(CaseLabels));
		}
		| CaseLabels {
            $$ = alloc_NONLEAF(CL_CaseLabelList, 83, 1, $1);
...
CaseLabels
		: ConstExpression RANGE ConstExpression {
            $$ = alloc_NONLEAF(CL_CaseLabels, 84, 3, $1, alloc_SYMBOL(RANGE, $2), $3);
			RULE(CaseLabels, NONTERM(ConstExpression) SYMBOL("..") NONTERM(ConstExpression));
		}
		| ConstExpression {
            $$ = alloc_NONLEAF(CL_CaseLabels, 85, 1, $1);
...
SetType
		: SET OF SimpleType {
            $$ = alloc_NONLEAF(CL_SetType, 86, 3, alloc_SYMBOL(SET, $1), alloc_SYMBOL(OF, $2), $3);
...
PointerType
		: POINTER TO type {
            $$ = alloc_NONLEAF(CL_PointerType, 87, 3, alloc_SYMBOL(POINTER, $1), alloc_SYMBOL(TO, $2), $3);
...
ProcedureType
		: PROCEDURE FormalTypeList {
            $$ = alloc_NONLEAF(CL_ProcedureType, 88, 2, alloc_SYMBOL(PROCEDURE, $1), $2);
			RULE(ProcedureType, KEYWORD(PROCEDURE) NONTERM(FormalTypeList));
		}
		| PROCEDURE {
            $$ = alloc_NONLEAF(CL_ProcedureType, 89, 1, alloc_SYMBOL(PROCEDURE, $1));
...
FormalTypeList
		: paren_formal_parameter_type_list_opt ':' qualident {
            $$ = alloc_NONLEAF(CL_FormalTypeList, 90, 3, $1, alloc_SYMBOL(':', $2), $3);
		...
		| paren_formal_parameter_type_list_opt {
            $$ = alloc_NONLEAF(CL_FormalTypeList, 91, 1, $1);
			RULE(FormalTypeList, NONTERM(paren_formal_parameter_type_list_opt));
		}
		;
...
paren_formal_parameter_type_list_opt
		: '(' formal_parameter_type_list_opt ')' {
            $$ = alloc_NONLEAF(CL_paren_formal_parameter_type_list_opt, 92, 3, alloc_SYMBOL('(', $1), $2, alloc_SYMBOL(')', $3));
...
formal_parameter_type_list_opt
		: formal_parameter_type_list {
            $$ = alloc_NONLEAF(CL_formal_parameter_type_list_opt, 93, 1, $1);
			RULE(formal_parameter_type_list_opt,
				NONTERM(formal_parameter_type_list_opt));
		}
		| /* EMPTY */ {
            $$ = alloc_NONLEAF(CL_formal_parameter_type_list_opt, 94, 0);
...
formal_parameter_type_list
		: formal_parameter_type_list ',' formal_parameter_type {
            $$ = alloc_NONLEAF(CL_formal_parameter_type_list, 95, 3, $1, alloc_SYMBOL(',', $2), $3);
		...
		| formal_parameter_type {
            $$ = alloc_NONLEAF(CL_formal_parameter_type_list, 96, 1, $1);
...
formal_parameter_type
		: VAR FormalType {
            $$ = alloc_NONLEAF(CL_formal_parameter_type, 97, 2, alloc_SYMBOL(VAR, $1), $2);
			RULE(formal_parameter_type, KEYWORD(VAR) NONTERM(FormalType));
		}
		| FormalType {
            $$ = alloc_NONLEAF(CL_formal_parameter_type, 98, 1, $1);
...
VariableDeclaration
		: IdentList ':' type {
            $$ = alloc_NONLEAF(CL_VariableDeclaration, 99, 3, $1, alloc_SYMBOL(':', $2), $3);
...
designator
		: designator '.' IDENT {
            $$ = alloc_NONLEAF(CL_designator, 100, 3, $1, alloc_SYMBOL('.', $2), alloc_IDENT($3));
			RULE(designator, NONTERM(designator) SYMBOL(".") TERMIN("IDENT:%s"), $3);
		}
		| designator '[' ExpList ']' {
            $$ = alloc_NONLEAF(CL_designator, 101, 4, $1, alloc_SYMBOL('[', $2), $3, alloc_SYMBOL(']', $4));
			RULE(designator, NONTERM(designator) SYMBOL("[") NONTERM(ExpList) SYMBOL("]"));
		}
		| designator '^' {
            $$ = alloc_NONLEAF(CL_designator, 102, 2, $1, alloc_SYMBOL('^', $2));
			RULE(designator, NONTERM(designator) SYMBOL("^"));
		}
		| qualident {
            $$ = alloc_NONLEAF(CL_designator, 103, 1, $1);
...
ExpList
		: ExpList ',' expression {
            $$ = alloc_NONLEAF(CL_ExpList, 104, 3, $1, alloc_SYMBOL(',', $2), $3);
			RULE(ExpList, NONTERM(ExpList) SYMBOL(",") NONTERM(expression));
		}
		| expression {
            $$ = alloc_NONLEAF(CL_ExpList, 105, 1, $1);
...
SimpleExpression
		: SimpleExpression AddOperator term {
            $$ = alloc_NONLEAF(CL_SimpleExpression, 108, 3, $1, $2, $3);
			RULE(SimpleExpression, NONTERM(SimpleExpression) NONTERM(AddOperator) NONTERM(term));
		}
		| add_op_opt term {
            $$ = alloc_NONLEAF(CL_SimpleExpression, 109, 2, $1, $2);
...
term
		: term MulOperator factor {
            $$ = alloc_NONLEAF(CL_term, 110, 3, $1, $2, $3);
			RULE(term, NONTERM(term) NONTERM(MulOperator) NONTERM(factor));
		}
		| factor {
            $$ = alloc_NONLEAF(CL_term, 111, 1, $1);
...
factor
		: INTEGER {
            $$ = alloc_NONLEAF(CL_factor, 112, 1, alloc_INTEGER($1));
			RULE( factor, TERMIN("INTEGER(%d)"), $1);
		}
		| REAL {
            $$ = alloc_NONLEAF(CL_factor, 113, 1, alloc_REAL($1));
			RULE(factor, TERMIN("REAL(%lg)"), $1);
		}
		| STRING {
            $$ = alloc_NONLEAF(CL_factor, 114, 1, alloc_STRING($1));
			RULE(factor, TERMIN("STRING(%s)"), $1);
		}
		| CHARLIT {
            $$ = alloc_NONLEAF(CL_factor, 115, 1, alloc_CHARLIT($1));
			RULE(factor, TERMIN("CHARLIT(\\%03o)"), $1);
		}
		| set {
            $$ = alloc_NONLEAF(CL_factor, 116, 1, $1);
			RULE(factor, NONTERM(set));
		}
		| designator ActualParameters {
            $$ = alloc_NONLEAF(CL_factor, 117, 2, $1, $2);
			RULE(factor, NONTERM(designator) NONTERM(ActualParameters));
		}
		| designator {
            $$ = alloc_NONLEAF(CL_factor, 118, 1, $1);
			RULE(factor, NONTERM(designator));
		}
		| '(' expression ')' {
            $$ = alloc_NONLEAF(CL_factor, 119, 3, alloc_SYMBOL('(', $1), $2, alloc_SYMBOL(')', $3));
			RULE(factor, SYMBOL("(") NONTERM(expression) SYMBOL(")"));
		}
		| NOT factor {
            $$ = alloc_NONLEAF(CL_factor, 120, 2, alloc_SYMBOL(NOT, $1), $2);
...
ActualParameters
		: '(' ExpList ')' {
            $$ = alloc_NONLEAF(CL_ActualParameters, 121, 3, alloc_SYMBOL('(', $1), $2, alloc_SYMBOL(')', $3));
			RULE(ActualParameters, SYMBOL("(") NONTERM(ExpList) SYMBOL(")"));
		}
		| '(' ')' {
            $$ = alloc_NONLEAF(CL_ActualParameters, 122, 2, alloc_SYMBOL('(', $1), alloc_SYMBOL(')', $2));
...
statement
		: assignment {
            $$ = alloc_NONLEAF(CL_statement, 123, 1, $1);
			RULE(statement, NONTERM(assignment));
		}
		| ProcedureCall {
            $$ = alloc_NONLEAF(CL_statement, 124, 1, $1);
			RULE(statement, NONTERM(ProcedureCall));
		}
		| IfStatement {
            $$ = alloc_NONLEAF(CL_statement, 125, 1, $1);
			RULE(statement, NONTERM(IfStatement));
		}
		| CaseStatement {
            $$ = alloc_NONLEAF(CL_statement, 126, 1, $1);
			RULE(statement, NONTERM(CaseStatement));
		}
		| WhileStatement {
            $$ = alloc_NONLEAF(CL_statement, 127, 1, $1);
			RULE(statement, NONTERM(WhileStatement));
		}
		| RepeatStatement {
            $$ = alloc_NONLEAF(CL_statement, 128, 1, $1);
			RULE(statement, NONTERM(RepeatStatement));
		}
		| LoopStatement {
            $$ = alloc_NONLEAF(CL_statement, 129, 1, $1);
			RULE(statement, NONTERM(LoopStatement));
		}
		| ForStatement {
            $$ = alloc_NONLEAF(CL_statement, 130, 1, $1);
			RULE(statement, NONTERM(ForStatement));
		}
		| WithStatement {
            $$ = alloc_NONLEAF(CL_statement, 131, 1, $1);
			RULE(statement, NONTERM(WithStatement));
		}
		| EXIT {
            $$ = alloc_NONLEAF(CL_statement, 132, 1, alloc_SYMBOL(EXIT, $1));
			RULE(statement, KEYWORD(EXIT));
		}
		| TRETURN expression {
            $$ = alloc_NONLEAF(CL_statement, 133, 2, alloc_SYMBOL(TRETURN, $1), $2);
			RULE(statement, KEYWORD(RETURN)NONTERM(expression));
		}
		| TRETURN {
            $$ = alloc_NONLEAF(CL_statement, 134, 1, alloc_SYMBOL(TRETURN, $1));
			RULE(statement, KEYWORD(RETURN));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_statement, 135, 0);
...
assignment
		: designator ASSIGN expression {
            $$ = alloc_NONLEAF(CL_assignment, 136, 3, $1, alloc_SYMBOL(ASSIGN, $2), $3);
...
ProcedureCall
		: designator ActualParameters {
            $$ = alloc_NONLEAF(CL_ProcedureCall, 137, 2, $1, $2);
			RULE(ProcedureCall, NONTERM(designator) NONTERM(ActualParameters));
		}
		| designator {
            $$ = alloc_NONLEAF(CL_ProcedureCall, 138, 1, $1);
...
StatementSequence
		: StatementSequence ';' statement {
            $$ = alloc_NONLEAF(CL_StatementSequence, 139, 3, $1, alloc_SYMBOL(';', $2), $3);
			RULE(StatementSequence, NONTERM(StatementSequence) SYMBOL(";") NONTERM(statement));
		}
		| statement {
            $$ = alloc_NONLEAF(CL_StatementSequence, 140, 1, $1);
...
IfStatement
		: IF expression THEN
		  ...
		  else_opt
		  END {
            $$ = alloc_NONLEAF(CL_IfStatement, 141, 7, alloc_SYMBOL(IF, $1), $2, alloc_SYMBOL(THEN, $3), $4, $5, $6, alloc_SYMBOL(END, $7));
...
elsif_seq
		: elsif_seq ELSIF expression THEN StatementSequence {
            $$ = alloc_NONLEAF(CL_elsif_seq, 142, 5, $1, alloc_SYMBOL(ELSIF, $2), $3, alloc_SYMBOL(THEN, $4), $5);
			RULE(elsif_seq, NONTERM(elsif_seq) KEYWORD(ELSIF)
				NONTERM(expression) KEYWORD(THEN) NONTERM(StatementSequence));
		}
		| /* EMPTY */ {
            $$ = alloc_NONLEAF(CL_elsif_seq, 143, 0);
...
else_opt
		: ELSE StatementSequence {
            $$ = alloc_NONLEAF(CL_else_opt, 144, 2, alloc_SYMBOL(ELSE, $1), $2);
			RULE(else_opt, KEYWORD(ELSE) NONTERM(StatementSequence));
		}
		| /* EMPTY */ {
            $$ = alloc_NONLEAF(CL_else_opt, 145, 0);
			RULE(else_opt, EMPTY);
		}
		;
...
CaseStatement
		: CASE expression OF
			...
			else_opt
		  END {
            $$ = alloc_NONLEAF(CL_CaseStatement, 146, 6, alloc_SYMBOL(CASE, $1), $2, alloc_SYMBOL(OF, $3), $4, $5, alloc_SYMBOL(END, $6));
...
case_list
		: case_list '|' case {
            $$ = alloc_NONLEAF(CL_case_list, 147, 3, $1, alloc_SYMBOL('|', $2), $3);
			RULE(case_list, NONTERM(case_list) SYMBOL("|") NONTERM(case));
		}
		| case {
            $$ = alloc_NONLEAF(CL_case_list, 148, 1, $1);
...
case
		: CaseLabelList ':' StatementSequence {
            $$ = alloc_NONLEAF(CL_case, 149, 3, $1, alloc_SYMBOL(':', $2), $3);
...
WhileStatement
		: WHILE expression DO
			StatementSequence
		  END {
            $$ = alloc_NONLEAF(CL_WhileStatement, 150, 5, alloc_SYMBOL(WHILE, $1), $2, alloc_SYMBOL(DO, $3), $4, alloc_SYMBOL(END, $5));
...
RepeatStatement
		: REPEAT
			StatementSequence
		  UNTIL expression {
            $$ = alloc_NONLEAF(CL_RepeatStatement, 151, 4, alloc_SYMBOL(REPEAT, $1), $2, alloc_SYMBOL(UNTIL, $3), $4);
...
ForStatement
		: FOR IDENT ASSIGN expression TO expression by_opt DO
			StatementSequence
		  END {
            $$ = alloc_NONLEAF(CL_ForStatement, 152, 10, alloc_SYMBOL(FOR, $1), alloc_IDENT($2), alloc_SYMBOL(ASSIGN, $3),
...
by_opt
		: BY ConstExpression {
            $$ = alloc_NONLEAF(CL_by_opt, 153, 2, alloc_SYMBOL(BY, $1), $2);
			RULE(by_opt, KEYWORD(BY) NONTERM(ConstExpression));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_by_opt, 154, 0);
			RULE(by_opt, EMPTY);
		}
		;
...
LoopStatement
		: LOOP
			StatementSequence
		  END {
            $$ = alloc_NONLEAF(CL_LoopStatement, 155, 3, alloc_SYMBOL(LOOP, $1), $2, alloc_SYMBOL(END, $3));
...
WithStatement
		: WITH designator DO StatementSequence END {
            $$ = alloc_NONLEAF(CL_WithStatement, 156, 5, alloc_SYMBOL(WITH, $1), $2, alloc_SYMBOL(DO, $3), $4, alloc_SYMBOL(END, $3));
...
ProcedureDeclaration
		: ProcedureHeading ';' block IDENT {
            $$ = alloc_NONLEAF(CL_ProcedureDeclaration, 157, 4, $1, alloc_SYMBOL(';', $2), $3, alloc_IDENT($4));
...
ProcedureHeading
		: PROCEDURE IDENT FormalParameters_opt {
            $$ = alloc_NONLEAF(CL_ProcedureHeading, 158, 3, alloc_SYMBOL(PROCEDURE, $1), alloc_IDENT($2), $3);
...
FormalParameters_opt
		: FormalParameters {
            $$ = alloc_NONLEAF(CL_FormalParameters_opt, 159, 1, $1);
			RULE(FormalParameters_opt, NONTERM(FormalParameters));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_FormalParameters_opt, 160, 0);
...
block
		: declaration_list_opt
		  ...
		  END {
            $$ = alloc_NONLEAF(CL_block, 161, 3, $1, $2, alloc_SYMBOL(END, $3));
...
declaration_list_opt
		: declaration_list_opt declaration {
            $$ = alloc_NONLEAF(CL_declaration_list_opt, 162, 2, $1, $2);
		...
		| /* empty */ {
             ...
             */
            $$ = alloc_NONLEAF(CL_declaration_list_opt, 163, 0);
...
BEGIN_StatementSequence_opt
		: TBEGIN StatementSequence {
            $$ = alloc_NONLEAF(CL_BEGIN_StatementSequence_opt, 164, 2, alloc_SYMBOL(TBEGIN, $1), $2);
			RULE(BEGIN_StatementSequence_opt, KEYWORD(BEGIN)
				NONTERM(StatementSequence));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_BEGIN_StatementSequence_opt, 165, 0);
...
ConstantDeclaration_list_opt
		: ConstantDeclaration_list_opt ConstantDeclaration ';' {
            $$ = alloc_NONLEAF(CL_ConstantDeclaration_list_opt, 171, 3, $1, $2, alloc_SYMBOL(';', $3));
			RULE(ConstantDeclaration_list_opt, NONTERM(ConstantDeclaration_list_opt)
				NONTERM(ConstantDeclaration) SYMBOL(";"));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_ConstantDeclaration_list_opt, 172, 0);
...
TypeDeclaration_list_opt
		: TypeDeclaration_list_opt TypeDeclaration ';' {
            $$ = alloc_NONLEAF(CL_TypeDeclaration_list_opt, 173, 3, $1, $2, alloc_SYMBOL(';', $3));
			RULE(TypeDeclaration_list_opt, NONTERM(TypeDeclaration_list_opt)
				NONTERM(TypeDeclaration) SYMBOL(";"));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_TypeDeclaration_list_opt, 174, 0);
...
VariableDeclaration_list_opt
		: VariableDeclaration_list_opt VariableDeclaration ';' {
            $$ = alloc_NONLEAF(CL_VariableDeclaration_list_opt, 175, 3, $1, $2, alloc_SYMBOL(';', $3));
			RULE(VariableDeclaration_list_opt, NONTERM(VariableDeclaration_list_opt)
				NONTERM(VariableDeclaration) SYMBOL(";"));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_VariableDeclaration_list_opt, 176, 0);
...
FormalParameters
		: '(' FPSection_list_opt ')' ':' qualident {
            $$ = alloc_NONLEAF(CL_FormalParameters, 177, 5, alloc_SYMBOL('(', $1), $2, alloc_SYMBOL(')', $3), alloc_SYMBOL(':', $4), $5);
			RULE(FormalParameters, SYMBOL("(") NONTERM(FPSection_list_opt)
				SYMBOL(")") SYMBOL(":") NONTERM(qualident));
		}
		| '(' FPSection_list_opt ')' {
            $$ = alloc_NONLEAF(CL_FormalParameters, 178, 3, alloc_SYMBOL('(', $1), $2, alloc_SYMBOL(')', $3));
			RULE(FormalParameters, SYMBOL("(") NONTERM(FPSection_list_opt)
				SYMBOL(")"));
		}
		;
...
FPSection_list_opt
		: FPSection_list {
            $$ = alloc_NONLEAF(CL_FPSection_list_opt, 179, 1, $1);
			RULE(FPSection_list_opt, NONTERM(FPSection_list));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_FPSection_list_opt, 180, 0);
...
FPSection_list
		: FPSection_list ';' FPSection {
            $$ = alloc_NONLEAF(CL_FPSection_list, 181, 3, $1, alloc_SYMBOL(';', $2), $3);
			RULE(FPSection_list, NONTERM(FPSection_list) SYMBOL(";") NONTERM(FPSection));
		}
		| FPSection {
            $$ = alloc_NONLEAF(CL_FPSection_list, 182, 1, $1);
...
FPSection
		: VAR IdentList ':' FormalType {
            $$ = alloc_NONLEAF(CL_FPSection, 183, 4, alloc_SYMBOL(VAR, $1), $2, alloc_SYMBOL(':', $3), $4);
			RULE(FPSection, KEYWORD(VAR) NONTERM(IdentList) SYMBOL(":") NONTERM(FormalType));
		}
		|     IdentList ':' FormalType {
            $$ = alloc_NONLEAF(CL_FPSection, 184, 3, $1, alloc_SYMBOL(':', $2), $3);
...
FormalType
		: ARRAY OF qualident {
            $$ = alloc_NONLEAF(CL_FormalType, 185, 3, alloc_SYMBOL(ARRAY, $1), alloc_SYMBOL(OF, $2), $3);
			RULE(FormalType, KEYWORD(ARRAY) KEYWORD(OF) NONTERM(qualident));
		}
		| qualident {
            $$ = alloc_NONLEAF(CL_FormalType, 186, 1, $1);
...
ModuleDeclaration
		: MODULE IDENT priority_opt ';'
			...
			export_opt
		  block IDENT {
            $$ = alloc_NONLEAF(CL_ModuleDeclaration, 187, 8, alloc_SYMBOL(MODULE, $1), alloc_IDENT($2), $3,
...
priority_opt
		: '[' ConstExpression ']' {
            $$ = alloc_NONLEAF(CL_priority_opt, 188, 3, alloc_SYMBOL('[', $1), $2, alloc_SYMBOL(']', $3));
			RULE(priority_opt, SYMBOL("[") NONTERM(ConstExpression) SYMBOL("]"));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_priority_opt, 189, 0);
...
import_list_opt
		: import_list_opt import {
            $$ = alloc_NONLEAF(CL_import_list_opt, 190, 2, $1, $2);
			RULE(import_list_opt, NONTERM(import_list_opt) NONTERM(import));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_import_list_opt, 191, 0);
...
export_opt
		: EXPORT QUALIFIED IdentList ';' {
            $$ = alloc_NONLEAF(CL_export_opt, 192, 4, alloc_SYMBOL(EXPORT, $1), alloc_SYMBOL(QUALIFIED, $2), $3, alloc_SYMBOL(';', $4));
			RULE(export_opt, KEYWORD(EXPORT) KEYWORD(QUALIFIED) NONTERM(IdentList) SYMBOL(";"));
		}
		| EXPORT IdentList ';' {
            $$ = alloc_NONLEAF(CL_export_opt, 193, 3, alloc_SYMBOL(EXPORT, $1), $2, alloc_SYMBOL(';', $3));
			RULE(export_opt, KEYWORD(EXPORT) NONTERM(IdentList) SYMBOL(";"));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_export_opt, 194, 0);
...
import
		: FROM IDENT IMPORT IdentList ';' {
            $$ = alloc_NONLEAF(CL_import, 195, 5, alloc_SYMBOL(FROM, $1), alloc_IDENT($2),
				alloc_SYMBOL(IMPORT, $3), $4, alloc_SYMBOL(';', $5));
			RULE(import, KEYWORD(FROM) TERMIN("IDENT:%s") KEYWORD(IMPORT)
				NONTERM(IdentList) SYMBOL(";"), $2);
		}
		|            IMPORT IdentList ';' {
            $$ = alloc_NONLEAF(CL_import, 196, 3, alloc_SYMBOL(IMPORT, $1), $2, alloc_SYMBOL(';', $3));
...
DefinitionModule
		: DEFINITION MODULE IDENT ';'
			...
			definition_list_opt
		  END IDENT {
            $$ = alloc_NONLEAF(CL_DefinitionModule, 197, 9, alloc_SYMBOL(DEFINITION, $1), alloc_SYMBOL(MODULE, $2),
...
definition_list_opt
		: definition_list_opt definition {
            $$ = alloc_NONLEAF(CL_definition_list_opt, 198, 2, $1, $2);
			RULE(definition_list_opt, NONTERM(definition_list_opt) NONTERM(definition));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_definition_list_opt, 199, 0);
...
definition
		: CONST ConstantDeclaration_list_opt {
            $$ = alloc_NONLEAF(CL_definition, 200, 2, alloc_SYMBOL(CONST, $1), $2);
			RULE(definition, KEYWORD(CONST) NONTERM(ConstantDeclaration_list_opt));
		}
		| TYPE opaque_type_list_opt {
            $$ = alloc_NONLEAF(CL_definition, 201, 2, alloc_SYMBOL(TYPE, $1), $2);
			RULE(definition, KEYWORD(TYPE) NONTERM(opaque_type_list_opt));
		}
		| VAR VariableDeclaration_list_opt {
            $$ = alloc_NONLEAF(CL_definition, 202, 2, alloc_SYMBOL(VAR, $1), $2);
			RULE(definition, KEYWORD(VAR) NONTERM(VariableDeclaration_list_opt));
		}
		| ProcedureHeading ';' {
            $$ = alloc_NONLEAF(CL_definition, 203, 2, $1, alloc_SYMBOL(';', $2));
...
opaque_type_list_opt
		: opaque_type_list_opt opaque_type {
            $$ = alloc_NONLEAF(CL_opaque_type_list_opt, 204, 2, $1, $2);
			RULE(opaque_type_list_opt, NONTERM(opaque_type_list_opt) NONTERM(opaque_type));
		}
		| /* empty */ {
            $$ = alloc_NONLEAF(CL_opaque_type_list_opt, 205, 0);
...
opaque_type
		: IDENT '=' type ';' {
            $$ = alloc_NONLEAF(CL_opaque_type, 206, 4, alloc_IDENT($1), alloc_SYMBOL('=', $2), $3, alloc_SYMBOL(';', $4));
			RULE(opaque_type, TERMIN("IDENT:%s") SYMBOL("=") NONTERM(type) SYMBOL(";"), $1);
		}
		| IDENT ';' {
            $$ = alloc_NONLEAF(CL_opaque_type, 207, 2, alloc_IDENT($1), alloc_SYMBOL(';', $2));
...
ProgramModule
		: MODULE IDENT priority_opt ';'
			import_list_opt
		  block IDENT {
            $$ = alloc_NONLEAF(CL_ProgramModule, 208, 7, alloc_SYMBOL(MODULE, $1), alloc_IDENT($2), $3,
